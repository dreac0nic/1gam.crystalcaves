...
public class VFirstPersonMovementController : MonoBehaviour
{
	...
	{
		RaycastHit hit_info;
		//Vector3 local_forward = (CameraAnchor ? CameraAnchor.forward : this.transform.forward);
		//Vector3 local_right = (CameraAnchor ? CameraAnchor.right : this.transform.right);

		// Perform ground check to see if we have left the ground.
		if(Physics.SphereCast(GroundCheckStart.position, GroundCheckRadius, GroundCheckDirection, out hit_info, (0.5f*m_Collider.height - m_Collider.radius) + GroundCheckDistance)) {
			if(!m_IsGrounded) {
				m_IsGrounded = true;
			}
		...
		if(m_MovementInput.magnitude > float.Epsilon) {
			...
			Vector3 movement_input = new Vector3(m_MovementInput.x, 0.0f, m_MovementInput.y);

			if(movement_input.magnitude > 1.0f) {
				movement_input.Normalize();
			}

			Vector3 target_velocity = MaximumVelocity*this.transform.TransformDirection(movement_input);
